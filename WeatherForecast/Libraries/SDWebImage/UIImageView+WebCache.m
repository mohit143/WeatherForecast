...
- (void)sd_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletionBlock)completedBlock {
    ...
    if ([self viewWithTag:999] && [[self viewWithTag:999] isKindOfClass:[UIActivityIndicatorView class]]) {
        act = (UIActivityIndicatorView*)[self viewWithTag:999];
    } else {
        act = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
        act.tag = 999;
        [self addSubview:act];
    }
    [self bringSubviewToFront:act];
    [act startAnimating];
    act.center = CGPointMake(self.frame.size.width/2, self.frame.size.height/2);
